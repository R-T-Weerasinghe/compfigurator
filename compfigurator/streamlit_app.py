...
def handle_task_input(prompt: str):
    ...
    if is_valid:
        ...
        add_message("assistant", f"Task set to: {validated_task}. Please enter your budget:")
    else:
...
def show_knowledge_base():
    """Display knowledge base in a modal dialog"""
    with st.expander("Knowledge Base Contents", expanded=True):
        col1, col2 = st.columns([10, 1])  # Create columns for content and close button
        with col2:
            if st.button("✖"):  # Close button
                st.session_state.show_kb = False
                st.rerun()
        # Create tabs for different sections
        tab1, tab2, tab3, tab4, tab5, tab6, tab7 = st.tabs([
            "Task Requirements", "Processors", "Motherboards", 
            "GPUs", "RAM", "Storage", "Monitors"
        ])
        
        with tab1:
            st.json(st.session_state.configurator.task_requirements)
        
        components = st.session_state.configurator.components
        with tab2:
            st.json(components.get("processors", []))
        with tab3:
            st.json(components.get("motherboards", []))
        with tab4:
            st.json(components.get("gpus", []))
        with tab5:
            st.json(components.get("ram", []))
        with tab6:
            st.json(components.get("storage", []))
        with tab7:
            st.json(components.get("monitors", []))


def main():
    st.title("Computer Configuration ES")
    init_session_state()
